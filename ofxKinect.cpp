...
//TODO: uncomment these when they are implemented in freenect
	...
	while (isThreadRunning())
	{
		if ( lock() )
		{
            ...
            nRetVal = kinectContext->WaitOneUpdateAll(depth);
...
int ofxKinect::openKinect(){
	...
	nRetVal = kinectContext->InitFromXmlFile(SAMPLE_XML_PATH, &errors);
	...
	nRetVal = kinectContext->FindExistingNode(XN_NODE_TYPE_DEPTH, depth);
	CHECK_RC(nRetVal, "Find depth generator");

	nRetVal = kinectContext->FindExistingNode(XN_NODE_TYPE_IMAGE, image);
